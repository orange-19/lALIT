...
def extract_headings_from_first_page(pdf_path):
    """
    Extract headings from the first page of a PDF.
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if is_heading:
                    ...
                    heading_info = {
                        ...
                        "top_position": round(top_position, 2),
                        "page": 1
                    }
                    headings.append(heading_info)
            
            # Sort headings by their position on the page (top to bottom)
            headings.sort(key=lambda x: x['top_position'])
...
def store_headings_separately(directory_path):
    """
    Process each PDF, extract headings, and store output separate    # Find all PDF files in the directory
    ...
    for pdf_file in pdf_files:
        ...
        headings = extract_headings_from_first_page(pdf_file)
        
        # Use filename as key
        filename = os.path.basename(pdf_file)
        if headings:
            # First page headings as the title
            title = " ".join([heading['text'] for heading in headings])
            outline = [
                {
                    "level": heading["level"],
                    "text": heading["text"],
            ...
            print(f"Saved {len(outline)} headings to {output_file}")

def main():
    parser = argparse.ArgumentParser(description="Extract headings from PDF files and save as JSON")
    ...
    if args.directory:
        ...
        # If specific PDF is provided, use its directory
    ...
    if args.pdf and os.path.exists(args.pdf):
        ...
        headings = extract_headings_from_first_page(args.pdf)
        ...
        store_headings_separately(directory)
